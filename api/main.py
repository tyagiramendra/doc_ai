...
from src.doc_analyzer.document_analyzer import DocumentAnalyzer
...
async def compare_documents(reference: UploadFile = File(...), actual: UploadFile = File(...)) -> Any:
    try:
        logger.info(f"Comparing files: {reference.filename} vs {actual.filename}")
...
async def chat_build_index(
    ...
    try:
        logger.info(f"Indexing chat session. Session ID: {session_id}, Files: {[f.filename for f in files]}")
        ...
        logger.info(f"Index created successfully for session: {ci.session_id}")
        return {"session_id": ci.session_id, "k": k, "use_session_dirs": use_session_dirs}
    except HTTPException:
        raise
    except Exception as e:
        logger.exception("Chat index building failed")
...
async def chat_query(
    ...
    try:
        logger.info(f"Received chat query: '{question}' | session: {session_id}")
        ...
        logger.info("Chat query handled successfully.")
