...
class DocumentComparatorLLM:
    def __init__(self):
        ...
        logger.info("DocumentComparatorLLM initialized", model=self.llm)
    ...
    def compare_documents(self, combined_docs: str) -> pd.DataFrame:
        try:
            ...
            logger.info("Invoking document comparison LLM chain")
            response = self.chain.invoke(inputs)
            logger.info("Chain invoked successfully", response_preview=str(response)[:200])
            return self._format_response(response)
        except Exception as e:
            logger.error("Error in compare_documents", error=str(e))
    ...
    def _format_response(self, response_parsed: list[dict]) -> pd.DataFrame: #type: ignore
        try:
            ...
            logger.error("Error formatting response into DataFrame", error=str(e))
